...
def train_embeddings(neg_list, pos_list, max_len):
    
    ## Balancing data.
    neg_array = np.asarray(neg_list)
    pos_array = np.asarray(pos_list)

    neg_array_len = len(neg_array)
    print("neg_array_len: ", neg_array_len)
    pos_array_len = len(pos_array)
    print("pos_array_len: ", pos_array_len)
    ...
    if neg_array_len > 0:
        ...
        if pos_array_len > 0:
            ...
            train_text_emb = np.concatenate((neg_array, pos_array), axis = 0)
            assert train_text_emb.shape == (neg_array_len + pos_array_len, max_len, 768)

            train_label_emb = np.array([0] * neg_array_len + [1] * pos_array_len)

        else:

            train_text_emb = neg_array
            assert train_text_emb.shape == (neg_array_len, max_len, 768)

            train_label_emb = np.array([0] * neg_array_len)

    else:

        if pos_array_len > 0:

            train_text_emb = pos_array
            assert train_text_emb.shape == (pos_array_len, max_len, 768)

            train_label_emb = np.array([1] * pos_array_len)
...
if __name__ == "__main__":
    ...
    parser.add_option("-m", "--max_len", action = "store", type = "int", dest = "max_len", help = "the maximum sentences number per document", default = 256)
    parser.add_option('-r', "--random_seed", type = 'int', action = 'callback', dest = 'random_seed', callback = list_callback, default = 1988)
    parser.add_option('-p', "--test_prop", type = 'float', action = 'store', dest = 'test_prop', help = "the proportion of the data used for testing", default = 0.2)
    ...
    print("proportion of data for testing: ", test_prop)
    ...
    ## We assume that there will be fewer nonviolation cases than violations.
    ## Hence we limit the number of violations to match the nonviolations.
    ## Since the data is stored as a dictionary, this will be random.
    data_non, data_vio = import_data(dataset, max_len)
    #data_vio = temp_vio[:len(data_non)]
    #assert len(data_non) == len(data_vio)
    """
    ## Setting data from January 2001
    data_non = data_non[:396]
    data_vio = data_vio[:2451]
    """
    ## Setting data from January 2015
    ...
    train_non, test_non = train_test_split(data_non, test_size = test_prop)
    train_vio, test_vio = train_test_split(data_vio, test_size = test_prop)

    ori_non_len = len(train_non)
    ori_vio_len = len(train_vio)

    if ori_non_len > ori_vio_len:
   
        original = copy.copy(train_vio)
        quo, didx = divmod(ori_non_len, ori_vio_len)
        #temp = np.tile(original,(quo,1))
        temp = np.repeat(original, repeats = quo, axis = 0)
        train_vio = np.concatenate((temp, original[:didx]), axis = 0)

    elif ori_vio_len > ori_non_len:

        original = copy.copy(train_non)
        print(original.shape)
        quo, didx = divmod(ori_vio_len, ori_non_len)
        temp = np.repeat(original, repeats = quo, axis = 0)
        print(temp.shape)
        train_non = np.concatenate((temp, original[:didx]), axis = 0)

    train_non_len = len(train_non)
    print("train_non_len: ", train_non_len)
    train_vio_len = len(train_vio)
    print("train_vio_len: ", train_vio_len)
    assert train_non_len == train_vio_len
    
    test_non_len = len(test_non)
    print("test_non_len: " + str(test_non_len))
    test_vio_len = len(test_vio)
    print("test_vio_len: " + str(test_vio_len))

    test_text_emb = np.concatenate((test_non, test_vio), axis = 0)
    assert test_text_emb.shape == (test_non_len + test_vio_len, max_len, 768)
    print(sizeof_fmt(sys.getsizeof(test_text_emb)))

    test_label_emb = np.array([0] * test_non_len + [1] * test_vio_len)
    X_test, y_test = test_text_emb, test_label_emb
    ...
    train_text_emb, train_label_emb = train_embeddings(train_non, train_vio, max_len)
    """
    ...
    X_train, y_train = train_text_emb, train_label_emb

    normalizer = Normalize()
    X_train = normalizer.normalize_train(X_train, max_len)
    X_test = normalizer.normalize_test(X_test, max_len)
    ...
    for e in tqdm(range(no_epochs)):
        ...
        for i, data in enumerate(trainloader):
            ...
            out = model(inputs)
            """ 
            ## Setting weights in accordance with classification distributions (that are heavily
            ## in favour of violations for article 6 - positive classifications in our usage).
            non_weight = train_vio_len / (train_non_len + train_vio_len)
            vio_weight = train_non_len / (train_non_len + train_vio_len)
            tense_weight = [non_weight, vio_weight]
            """
            tense_weight = [1.0, 1.0]            
            weight = torch.tensor(tense_weight).cuda(args['cuda_num'])
        ...
        with torch.no_grad():
            ...
            train_fact_atten_scores = torch.Tensor()
            ...
            print('Epoch: ', e, ' step: ', i, ' training MCC: ', train_epoch_mcc)
            print('Epoch: ', e, ' step: ', i, ' training confusion matrix: ', 'TP',train_tp, 'TN',train_tn, 'FP',train_fp, 'FN',train_fn)

            #train_attention_scores[fidx] = torch.cat((train_neg_atten_scores[:train_neg_len], train_pos_atten_scores[:train_pos_len]))

            ## Analysis of test data
            y_eval_pred = []
            y_eval_true = [0] * test_non_len + [1] * test_vio_len
            ...
            #test_attention_scores[fidx] = torch.cat((test_neg_atten_scores, test_pos_atten_scores))
            #print("len(y_eval_pred): ", len(y_eval_pred))
            #print("len(y_eval_true): ", len(y_eval_true))
            ...
            test_epoch_mcc = matthews_corrcoef(y_eval_true, y_eval_pred)
            test_mccs.append(test_epoch_mcc)
            
            test_confusion_matrices.append({'TP':tp, 'TN':tn, 'FP': fp, 'FN':fn})
            
            print("Epoch: ", e, " test analatics: ")
            print(classification_report(y_eval_true, y_eval_pred), "acc: ", acc, "f1: ", f_score, "mcc: ", test_epoch_mcc)
            macro_f.append(f_score)
            
            """
            ## Recording best attention scores and ascription models.
            if config.output_attentions:
            
                attention_output_dirs = os.path.join(args['attention_output_dir'],'%s'%(dataset), save_name)
                best_models_dirs = os.path.join(args['output_dir'],'%s'%(dataset), save_name)
                ...
                if test_best_mcc < test_epoch_mcc:
                    ...
                    files = glob.glob(attention_output_dirs + '*_factor_%s.pt'%(fidx))
                    ...
                    for fidx in range(base_factors):
                        newPath = shutil.copy("model_%s.p"%(fidx), best_models_dirs + 'epoch%s_factor_%s.pt'%(e, fidx))
                    
                    ## Storing best attentions.
                    for f in os.listdir(attention_output_dirs):
                        os.remove(os.path.join(attention_output_dirs, f))
    
                    for fidx in range(base_factors):
                        
                        train_factor_ascribe[fidx] = y_train_pred
                        test_factor_ascribe[fidx] = y_eval_pred
                    
                        doc_attention_score = []
                        attention_scores = torch.cat((train_attention_scores[fidx], test_attention_scores[fidx])) 
                        
                        for batch in attention_scores[fidx]:
                        
                            for doc in batch:
                                doc_attention_score.append(doc[0])
                                
                        data_len = train_neg_len + train_pos_len + test_neg_len + test_pos_len        
                        final_attention_scores = np.zeros((data_len, config.seq_length))
                        ...
                        torch.save(attention_scores, attention_output_dirs + 'epoch%s_factor_%s.pt'%(e, fidx))
